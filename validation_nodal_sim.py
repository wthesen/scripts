'''
    ...
    Example: Validating a nodal simulation
'''
...
def invalidate_conditions(model):
    conditions = {Parameters.NodalAnalysisSimulation.MAXLIQUIDRATE: -30000, Parameters.NodalAnalysisSimulation.USEPHASERATIO: True}
    model.tasks.nodalanalysis.set_conditions(producer="Well2 - Oil", study = "Study 1", parameters = conditions)
...
def make_sure_model_has_no_issues(model):
    ...
    issues = model.tasks.nodalanalysis.validate(producer="Well2 - Oil")
    if len(issues) > 0:
